...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const getUserAndProfile = async () => {
      ...
      if (user) {
        ...
        setSubscription({ tier: profileData?.subscription_tier || 'free' });
  ...
  const handleUpgradeToPro = async () => {
    if (subscription?.tier === 'pro' || subscription?.tier === 'enterprise') {
      setError('You already have a pro or enterprise subscription!');
      return;
    }
    ...
    <main className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-black to-black relative overflow-x-hidden p-3 sm:p-4 lg:p-8">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <div className="space-y-6 sm:space-y-8">
          ...
          <section className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-amber-500 to-orange-500 rounded-2xl sm:rounded-3xl blur opacity-20 group-hover:opacity-30 transition duration-300"></div>
            <div className="relative bg-gradient-to-br from-gray-900/95 to-black/95 backdrop-blur-2xl border border-amber-500/30 rounded-2xl sm:rounded-3xl shadow-2xl p-6 sm:p-8 hover:border-amber-400/50 transition-all duration-300">
              <div className="flex items-center gap-3 mb-6">
                <div className="relative">
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-500 to-orange-500 rounded-xl blur-md opacity-60"></div>
                  <div className="relative w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center shadow-lg">
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                    </svg>
                  </div>
                </div>
                <h2 className="text-xl sm:text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400">Subscription</h2>
              </div>

              <div className="grid sm:grid-cols-2 gap-4">
                {/* Current Plan */}
                <div className="bg-gradient-to-br from-amber-500/10 to-orange-500/10 rounded-2xl border border-amber-500/20 p-6 hover:border-amber-500/40 transition-all duration-200">
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <h3 className="text-lg font-bold text-white mb-1">Current Plan</h3>
                      <p className="text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-orange-400 capitalize">
                        {subscription?.tier === 'pro' ? '✨ piqo Pro' : subscription?.tier === 'enterprise' ? '🚀 Enterprise' : 'Free'}
                      </p>
                    </div>
                  ...
                  {subscription?.tier === 'free' && (
                    <p className="text-sm text-gray-300">Enjoy 1 free piqo with all core features</p>
                  )}
                  {subscription?.tier === 'pro' && (
                ...
                {/* Pro Features */}
                {subscription?.tier === 'free' && (
                  <div className="bg-gradient-to-br from-purple-500/10 to-pink-500/10 rounded-2xl border border-purple-500/20 p-6 hover:border-purple-500/40 transition-all duration-200">
                    <h3 className="text-base font-bold text-white mb-4">Unlock With piqo Pro</h3>
                    <ul className="space-y-3">
                      <li className="flex items-center gap-3">
                        ...
                        <span className="text-sm text-gray-300">Unlimited piqos</span>
                      </li>
                      <li className="flex items-center gap-3">
                        <svg className="w-4 h-4 text-purple-400 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                    ...
                    <button
                      ...
                      className="w-full mt-4 px-4 py-3 rounded-xl bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 disabled:from-gray-600 disabled:to-gray-700 disabled:cursor-not-allowed text-white text-sm font-bold transition-all duration-200 hover:shadow-2xl hover:shadow-purple-500/50 shadow-lg active:scale-95"
                    >
                      {checkingOut ? (
